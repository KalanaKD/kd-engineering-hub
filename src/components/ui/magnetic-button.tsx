...
export function magneticButtonClasses(
...
export function MagneticContent({ children, magnetic = true, variant = "primary" }: InnerProps) {
  ...
  return (
    <>
      ...
      {variant === "primary" && (
        <span
          aria-hidden
          className="pointer-events-none absolute inset-0 -z-10 rounded-full opacity-0 blur-xl transition-opacity duration-500 group-hover:opacity-60 bg-[radial-gradient(closest-side,var(--accent-primary),transparent)]"
        />
      )}
    </>
  );
...
export const MagneticButton = forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = "primary", children, magnetic = true, ...props }, ref) => (
    <button
      ref={ref}
      className={magneticButtonClasses(variant, className)}
      {...props}
    >
      <MagneticContent magnetic={magnetic} variant={variant}>
        {children}
      </MagneticContent>
    </button>
  ),
...
export const MagneticLink = forwardRef<HTMLAnchorElement, LinkProps>(
  ({ className, variant = "primary", children, magnetic = true, ...props }, ref) => (
    <a ref={ref} className={magneticButtonClasses(variant, className)} {...props}>
      <MagneticContent magnetic={magnetic} variant={variant}>
        {children}
      </MagneticContent>
    </a>
